 ...
 * available at http://www.broadcom.com/lcenses/GPLv2.php, with the
...
void dhd_write_mac_address(dhd_pub_t * dhdp)
{
	...
	if (strlen(mac_param))
	{
		...
		DHD_ERROR(("%s: converted mac value is %02x:%02x:%02x:%02x:%02x:%02x\n",
			  __FUNCTION__,
	...
	{
		DHD_ERROR(("Warning %s: mac_param is NULL \n", __FUNCTION__));
	}
}
/*porting,WIFI Module,20111110 end-- */
