...
bool g_screen_touch_event = false;
static char g_limit_speed_last_count = 0;
static bool g_limit_speed_flag = 0;

static void mtp_detect_touch_func(struct work_struct *work);
static DECLARE_DELAYED_WORK(mtp_detect_touch_event, mtp_detect_touch_func);

static void mtp_detect_touch_func(struct work_struct *work)
{
    if( g_screen_touch_event )
    {
        g_screen_touch_event = false;
        g_limit_speed_flag = true;
        g_limit_speed_last_count = 1;
    }

    if( g_limit_speed_flag )
    {/* limit speed for 500ms * 10 */
        g_limit_speed_last_count = (g_limit_speed_last_count + 1)%10;
    }

    if( 0 == g_limit_speed_last_count )
    {
        g_limit_speed_flag = false;
    }

    schedule_delayed_work(&mtp_detect_touch_event, msecs_to_jiffies(500));
