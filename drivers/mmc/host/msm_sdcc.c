...
/* DSThkf32834 huangxianghe strat polling mode, begin 2011/3/15 */
#ifndef CONFIG_MMC_MSM_CARD_HW_DETECTION
		mmc->caps |= MMC_CAP_NEEDS_POLL;
#endif
/* DSThkf32834 huangxianghe strat polling mode, end 2011/3/15 */
	/*
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if (mmc->card && mmc_card_sdio(mmc->card)) {
			...
			mmc->ios.clock = 0; // disable the clock for low consume
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		if(!(mmc->card && mmc_card_sdio(mmc->card))) {
			...
			pr_debug("%s: %s: ignore mmc_resume_host for wlan\n", mmc_hostname(mmc), __func__);
		}
...
static int msmsdcc_pm_suspend(struct device *dev)
{
	...
	if (!pm_runtime_suspended(dev) || (mmc->card && mmc_card_sdio(mmc->card)))
