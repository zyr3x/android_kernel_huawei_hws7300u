...
int get_remaining_capacity(struct bq275x0_device_info *di)
{
	int data = 0;
	data = bq275x0_i2c_read_word(di,BQ275x0_REG_RCAP);
	BQ275x0_DBG("remaining capacity  = %d mAH\n",data);
	return data;
}

#define BQ275x0_REG_FCC 0x12
int get_full_charge_capacity(struct bq275x0_device_info *di)
{
	int data = 0;
	data = bq275x0_i2c_read_word(di,BQ275x0_REG_FCC);
	BQ275x0_DBG("full charge capacity  = %d mAH\n",data);
	return data;
}
...
{
        return (bq275x0_battery_capacity(di) == 100);
}
...
bq275x0_firmware_program_begin:
    ...
    while(1)
    {
        ...
        /*\BB\F1ȡһ\D0\D0\CA\FD\BEݣ\ACȥ\B3\FD\BFո\F1*/
...
static u8 get_child_version(void)
{
    ...
    data[0] = 0x39;
    if(bq275x0_i2c_bytes_write(g_battery_measure_by_bq275x0_i2c_client, 0x3e, data, 1))
		return 0;
    mdelay(2);

    data[0] = 0x00;
    if(bq275x0_i2c_bytes_write(g_battery_measure_by_bq275x0_i2c_client, 0x3f, data, 1))
		return 0;
    mdelay(2);

    data[0] = 0x00;
    if(bq275x0_i2c_bytes_write(g_battery_measure_by_bq275x0_i2c_client, 0x61, data, 1))
		return 0;
    mdelay(2);

    bq275x0_i2c_bytes_read(g_battery_measure_by_bq275x0_i2c_client, 0x60, data, 1);
    mdelay(2);
...
/* zhouhenglin add for Firmware upgrade 20110117 begine */ 
    ...
    printk(KERN_ERR "[%s,%d] Battery support disabled\n",__FUNCTION__,__LINE__);
	retval = -ENOMEM;
	goto batt_failed_0;
	}
    else
