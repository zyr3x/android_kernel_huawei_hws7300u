...
DECLARE_COMPLETION(bt_sync_completion);
...
void bluesleep_sleep_wakeup(void)
{
	if (test_bit(BT_ASLEEP, &flags)) {
		...
		gpio_set_value(bsi->ext_wake, GPIO_LOW);
		clear_bit(BT_ASLEEP, &flags);
		/*Activating UART */
		hsuart_power(POWER_ON);
	}
    else 
	{
	    /* only effective when BroadCom BT chip is uesed */
        int bt_wake = GPIO_LOW;

        mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL * HZ));

        bt_wake = gpio_get_value(bsi->ext_wake);
        
        if(GPIO_HIGH== bt_wake) 
		{
            gpio_set_value(bsi->ext_wake, GPIO_LOW);
        }
        //hsuart_power(POWER_ON);
				...
				unsigned long event, void *data)
{
	...
	case HCI_DEV_REG:
		...
		break;
	case HCI_DEV_UNREG:
		if(unlikely(!wait_for_completion_timeout(&bt_sync_completion, 3*HZ))){
			printk(KERN_ERR "%s:wait_for_completion_timeout \n",__func__);
		}
		bluesleep_hdev = NULL;
		bsi->uport = NULL;
		break;
					...
					int count, int *eof, void *data)
{
    ...
    /* judge the 2 paras. start and data are not used, and NULL is acceptable*/
    if ((NULL == page) || (NULL == eof)) 
    {
        BT_ERR("[bluepower_read_proc_btwake@bluesleep.c]-000 eof is a NULL pointer!! \n");
        return 0;
    }
...
{
    /*
     * judge whether the point is NULL 
     */
    unsigned int proto = 0;
    /* judge the 2 paras. start and data are not used, and NULL is acceptable*/
	if ((NULL == page) || (NULL == eof))
    {
        BT_ERR("[bluesleep_read_proc_proto@bluesleep.c]-000 eof is a NULL pointer!! \n");
        return 0;
    }
					...
					unsigned long count, void *data)
{
	...
	if (proto == '0'){
		...
		if(unlikely(NULL == bsi->uport)){
			printk(KERN_ERR " %s:NULL == bsi->uport \n",__func__);
			return -EINVAL;
		}
		bluesleep_stop();
		complete(&bt_sync_completion);
